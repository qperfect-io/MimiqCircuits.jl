...
When set to `true`, the default method (`:sa_warm_start`) is used. You can also pass a `Symbol` or `String` to select a specific reordering algorithm:
...
- `:memetic` — Evolutionary algorithm with local search. Restricted to circuits with **≤50 qubits**.
- `:multilevel` — Multi-level coarsening and refinement algorithm.
- `:grasp` — GRASP (Greedy Randomized Adaptive Search Procedure) with path relinking. Restricted to circuits with **≤50 qubits**.
- `:hybrid` — Multi-level + GRASP hybrid for best quality at higher computational cost. Restricted to circuits with **≤50 qubits**.
...
Independent seed for the qubit reordering random number generator. When set, uses a separate `MersenneTwister` seeded with this value for the reordering algorithm, instead of deriving the RNG from the main simulation `seed`.
...
**Default:** `nothing` (backend dependent)
